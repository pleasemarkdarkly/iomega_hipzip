// derived.cpp --
// $Id: derived.cpp,v 1.7 2001/12/18 09:43:45 wcvs Exp $
...
private:
  ...
  static int qs_compare(const void* a, const void* b);
  int do_qs_compare(const void* a, const void* b);
  virtual int Compare(int, c4_Cursor) const;
...
static c4_SortSeq* pCur;
void c4_SortSeq::QuickSort( T ar[], int size )
{
    pCur = this;
    qsort( (void*) ar, size, sizeof(T), c4_SortSeq::qs_compare);
}
int c4_SortSeq::qs_compare( const void* a, const void* b ) 
{
    return pCur->do_qs_compare( a, b );
}
int c4_SortSeq::do_qs_compare( const void* a, const void* b ) 
{
    T c = *(T*)a, d = *(T*)b;
    if( c == d ) return 0;
    c4_SortInfo* info;
    for( info = _info; info->_handler ; info++ ) {
        int f = info->CompareOne(_seq, c, d);
        if( f ) {
            int n = info - _info;
            if( _width < n )
                _width = n;
            return (_down.Contents()[n] ? -f : f );
        }
    }
    _width = info - _info;
  ...
  : c4_FilterSeq (seq_), _info (0), _width (-1)
{
  ...
  if (NumRows() > 0)
  {
    ...
    // dc- everything is ready to go except your brain. use qsort.
    QuickSort((T*) &_rowMap.ElementAt(0), NumRows());
