...
SDVOID pc_verify_blk(BLKBUFF *pblk)
{
    ...
    {        
        ...
        if(directory_buffer[i].sigstart != BLOCK_START_SIG)
        {
            diag_printf("invalid start signature 0x%8.8x on block %d\n",directory_buffer[i].sigstart,i);
            err = 1;
            break;
        }

        if(directory_buffer[i].sigend != BLOCK_END_SIG)
        {
            diag_printf("invalid end signature 0x%8.8x on block %d\n",directory_buffer[i].sigend,i);
            err = 1;
            break;
        }
    }

    DBASSERT( FAT, err == 0, "Block buffer corruption - fatal error\n");
...
{
#ifdef USE_MALLOC
    BLKBUFF* pblk = (BLKBUFF*)malloc(sizeof(BLKBUFF));
    memset( (void*)pblk, 0, sizeof(BLKBUFF) );
    pblk->data = (UINT16*) malloc(512);
    memset( (void*)pblk->data, 0, 512 );
    useindex += 1L;
    pblk->lru = useindex;
    pblk->pdrive = pdrive;
...
SDVOID pc_free_all_blk(DDRIVE *pdrive) /*__fn__*/
{
    BLKBUFF *pblk;
#ifdef USE_MALLOC
    diag_printf("pc_free_all_blk called!!!\n");
    return ;
#endif
