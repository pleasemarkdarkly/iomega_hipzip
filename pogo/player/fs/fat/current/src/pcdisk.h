...
typedef struct finode {
    ...
    /*** end of restricted portion of structure ***/
    BLOCKT  my_block;
...
#define BLOCK_START_SIG 0xAAAA5555

#define BLOCK_END_SIG 0x5555AAAA
...
// don't break fat magic
typedef struct fat_block_alloc {
    UINT16   core[256];
} FAT_BLOCK_ALLOC;
...
typedef struct blkbuff {
    UINT16 *data;
    BLOCK_ALLOC *datablock;      /* Pointer to the buffer area */
...
typedef struct _BOOTSECTCOMMON {
...
} __attribute__((packed)) BOOTSECTCOMMON, *PBOOTSECTCOMMON;


// starting at offset 36
typedef struct _BOOTSECT16
{
...
} __attribute__((packed)) BOOTSECT16, *PBOOTSECT16;
...
typedef struct _BOOTSECT32
{
    ...
    TEXT    bsVolumeLabel[11]; /* volume label */
    TEXT    bsFileSysType[8]; /* file-system type (FAT12, FAT16, FAT32)  */
} __attribute__((packed)) BOOTSECT32, *PBOOTSECT32;
...
SDIMPORT FAT_BLOCK_ALLOC fat_drives[FAT_BUFFER_SIZE*NDRIVES];
...
SDVOID pc_verify_blk(BLKBUFF *pblk);
...
ULONG    pc_ifree_clusters(INT16 driveno, UINT16 *bytespcluster);
...
SDBOOL  devio_read(INT16 driveno, ULONG blockno, UCHAR *buf, ULONG n_to_read);
SDBOOL  devio_write(INT16 driveno, ULONG blockno, UCHAR *buf, ULONG n_to_write);
...
/* errcode.c */
INT16   platform_convert_critical_error(INT16 err);
