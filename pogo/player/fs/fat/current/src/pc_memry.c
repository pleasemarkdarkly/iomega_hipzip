...
SDBOOL _pc_memory_init ( SDVOID ) /*__fn__*/
{
...
#if SANDISK_DOESNT_REALIZE_THAT_CONTROLLERS_ARE_NOT_MEMORY
    ...
    */
    ...
    for (i = 0, j = 1; i < (NBLKBUFFS-1); i++, j++)
    {
        ...
        mem_block_pool[i].data = directory_buffer[i].core;
        mem_block_pool[i].datablock = &directory_buffer[i];
        /* sign blocks for later validation */
        directory_buffer[i].sigstart = BLOCK_START_SIG;
        directory_buffer[i].sigend = BLOCK_END_SIG;
    ...
    mem_block_pool[NBLKBUFFS-1].data = directory_buffer[(NBLKBUFFS-1)].core;
    mem_block_pool[(NBLKBUFFS-1)].datablock = &directory_buffer[(NBLKBUFFS-1)];
    directory_buffer[(NBLKBUFFS-1)].sigstart = BLOCK_START_SIG;
    directory_buffer[(NBLKBUFFS-1)].sigend = BLOCK_END_SIG;
