...
{
    IFileNameRef*  pFilename;          //!< The unique URL of this media record
...
{
    IFileNameRef*  pFilename;          //!< The unique URL of this playlist record
    bool    bVerified;      //!< True if this record has been verified to exist on the data source
...
public:
    ...
    virtual void CompareFileSystemUpdate( CFileNameStore* store ) = 0;
    virtual CFileNameStore* GetFileNameStore() = 0;
    ...
    virtual IMediaContentRecord* GetMediaRecord(IFileNameRef* file) = 0;
...
typedef enum ContentRecordStatus {
    ...
    virtual IFileNameRef* GetFileNameRef() const = 0;
    //! Sets the file name reference
    virtual void SetFileNameRef(IFileNameRef*) = 0;
