...
#include <util/debug/debug.h>
DEBUG_MODULE_S(DBG_FILE_CACHE, DBGLEV_DEFAULT | DBGLEV_INFO );
...
void CFileCache::InitForFileNameRef(IFileNameRef* file)
{
    CloseFile();
	SetFileNameRef(file);
    Initialize();
...
// minimize, but also detach from the file ref.
void CFileCache::Deconfigure()
{
    Minimize();
    // (epg,11/30/2001): TODO: will a filecache get its own ref, or just a ptr copy?
...
void CFileCache::PrintLocationReport(const char* caption)
{
    DEBUGP( DBG_FILE_CACHE, DBGLEV_INFO, "fc:Location Report '%s'\n",caption);
    DEBUGP( DBG_FILE_CACHE, DBGLEV_INFO, "FileOff  BufOff EOF  Start    SkipSt  SkipExt  EmptyIdx  FullIdx  \n");
    DEBUGP( DBG_FILE_CACHE, DBGLEV_INFO, "%08d %05d  %03d  %08d %03d     %03d      %03d       %03d\n",
    //                                    xxxxxxxx xxxxx  xxx  xxxxxxxx xxx     xxx      xxx       xxx                                       
        m_nFileOffset, m_nBufferOffset, m_nPartialIndex, m_nStartChar, m_nSkipStart, m_nSkipExtent, m_nNextEmptyBuffer->Get(), m_nNextFullBuffer->Get());
}
...
bool CFileCache::ForwardFillForSeek(int nFileOffset)
{
	...
	if (nBufsAway <= BUFFERS_TO_PREBUFFER)
	{
        ...
        PrintLocationReport("ForwardFill");
        return true;
    }
    return false;
}
...
void CFileCache::FillBuffer()
{
    ...
    DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:F %d:%d\n",CCacheMan::GetInstance()->BufferIndexFromLocation(pBuffer), 
        FilePositionFromBufferListIndex(m_nNextEmptyBuffer->Get()));

	// read from the file
    //DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "rd %d>%d\n",(int)m_pFile,(int)pBuffer);
	int result = m_pFile->Read((void*)pBuffer, BUFFER_SIZE);
    //DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "k\n");
	...
	if (result != BUFFER_SIZE)
	{
		...
		if (result <= 0)
		{
            // this shouldn't happen: we predict the eof buffer, so we won't try this.  pee if we do.
            DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "FC:trying to read past eof\n");
    ...
    else if (m_nPartialIndex == m_nNextEmptyBuffer->Get())
    {
        m_bEOF = true;
        m_nPartialChars = BUFFER_SIZE;
        m_nPartialIndex = m_nNextEmptyBuffer->Get();
        CloseFile();
    }
    ...
    // increment the next empty counter.  at eof, this will put it off the end of the list, but the access function should deal with that case, and it keeps us conceptually consistent.
    IncrementNextEmptyBuffer();
    return;
}
...
int CFileCache::GiveUpBuffers(int nRequested, bool bFromHead)
{
    ...
    {
        if (BufferCountFromCharCount(m_nChars) < nRequested)
        {
            Minimize();
            return nRequested;
        }
    ...
    else
    {
        ...
        while (m_nNextEmptyBuffer->Get() > m_lstBuffers.Size() - nNeeded - 1)
        {
            m_nChars -= BUFFER_SIZE;
            DecrementNextEmptyBuffer();
        }
        for (int i = 0; i < nNeeded; ++i)
            lstFreed.PushBack(m_lstBuffers.PopBack());
        // we may have invalidated the itr at this point, so refresh it as possible.
        FixNextEmptyBuffer();
        CCacheMan::GetInstance()->AppendFreeBuffers(&lstFreed);
        return nRequested;
    }
...
void CFileCache::SetFileNameRef(IFileNameRef* file)
{
    delete m_pFileNameRef;
    if (!file)
        m_pFileNameRef = 0;
...
{
    if (!m_bFileOpen && m_pFileNameRef)
    {
        m_bFileOpen = true;
        if (m_pFile)
            delete m_pFile;
        const char* url = m_pFileNameRef->URL();
        m_pFile = CDataSourceManager::GetInstance()->OpenInputStream(url);
        if (m_pFileNameRef->DynamicAlloc())
            delete [] url;
        if (!m_pFile)
        {
            m_bFileCorrupt = true;
...
void CFileCache::IncrementNextFullBuffer()
{
    ...
    DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:IncFull %d\n",m_nNextFullBuffer->Get());
}
...
void CFileCache::DecrementNextFullBuffer()
{
    if (m_bNextFullOffEnd)
    {
        ...
        DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:DecFull %d AOE\n",m_nNextFullBuffer->Get());
    }
    else if (m_itNextFullBuffer != m_lstBuffers.GetHead())
    {
        --m_itNextFullBuffer;
        m_nNextFullBuffer->Dec();
        DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:DecFull %d\n",m_nNextFullBuffer->Get());
    }
}
...
void CFileCache::IncrementNextEmptyBuffer()
{
    ...
    DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:IncEmpty %d\n",m_nNextEmptyBuffer->Get());
}
...
void CFileCache::DecrementNextEmptyBuffer()
{
    if (m_bNextEmptyOffEnd)
    {
        ...
        DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:DecEmpty %d (%d) AOE\n",m_nNextEmptyBuffer->Get(),CCacheMan::GetInstance()->BufferIndexFromLocation(*m_itNextEmptyBuffer));
    }
    else if (m_itNextEmptyBuffer != m_lstBuffers.GetHead())
    {
        --m_itNextEmptyBuffer;
        m_nNextEmptyBuffer->Dec();
        DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:DecEmpty %d (%d)\n",m_nNextEmptyBuffer->Get(),CCacheMan::GetInstance()->BufferIndexFromLocation(*m_itNextEmptyBuffer));
    }
    else
    {
        DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "FC:Asked to Dec Empty already at head %d (&%d)\n",m_nNextEmptyBuffer->Get(),(*m_itNextEmptyBuffer));
    }
}

void CFileCache::FixNextEmptyBuffer()
{
    DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:FixEmpty %d\n",m_nNextEmptyBuffer->Get());
    ...
    if (m_itNextEmptyBuffer != m_lstBuffers.GetEnd())
        ...
        DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "FC:FixEmpty still zero!\n");
    }
}
...
void CFileCache::PrependBuffersForBOF()
{
    ...
    m_nFileOffset = 0;
    m_nChars = 0;
    // skip buffers if we have any left.
    if (nPrepend < m_lstBuffers.Size())
    {
        // figure out how many of the past-most buffers we'll get to keep, that the producer can "skip" during filling operations.
        ...
        if (m_nSkipStart + m_nSkipExtent > m_lstBuffers.Size())
            m_nSkipExtent = m_lstBuffers.Size() - m_nSkipStart;
	    // if we're skipping the eof buffer, we need to track that so the producer can restore that info later when it skips the eof buf.
        // we can't leave the m_bEOF flag on in the cache, or it won't fill right.  OTOH, we can leave the partial_buffer_chars count alone,
        // and it will just be reactivated as-is.
	    short nLastSkipBuffer = m_nSkipStart + m_nSkipExtent - 1;
    ...
    m_bEOF = false;
    m_pFile->Seek(IInputStream::SeekStart, 0);
...
void CFileCache::RewindCacheToBOF()
{
    ...
    // first back up to the start of current buffers.
...
{
    DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:ResetFull\n");
    m_nNextFullBuffer->Set(0);
...
{
    DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:ResetEmpty\n");
	m_nNextEmptyBuffer->Set(0);
    m_itNextEmptyBuffer = m_lstBuffers.GetHead();
    m_bNextEmptyOffEnd = false;
}
...
{
    if (m_nFileLength == -1 && (m_pFileNameRef))
    {
        OpenFile();
...
{
    DEBUGP( DBG_FILE_CACHE, DBGLEV_TRACE, "fc:Full %d %d\n",CountFullBuffers(),m_lstBuffers.Size());
