...
CSystemMessageScreen::SetScreenData(eSystemMessage eSysMesg, const TCHAR* pszMesg)
{
	...
	switch(eSysMesg)
	{
		case SHUTTING_DOWN:
			...
			break;
		case LOW_POWER:
			m_pString->DataSet(LS(SID_LOW_POWER));
			ChildRect.Set(mReal.wRight - 13, mReal.wBottom - 8, mReal.wRight - 1, mReal.wBottom);
			pBitmap = &gb_Battery_Empty_Bitmap;
			break;
		case USB_CONNECTED:
			m_pString->DataSet(LS(SID_USB_CONNECTED));
			ChildRect.Set(mReal.wLeft + 44, mReal.wTop + 35, mReal.wLeft + 84, mReal.wTop + 45);
			pBitmap = &gbUSBBitmap;
			break;
		case PLAYER_LOCKED:
			m_pString->DataSet(LS(SID_PLAYER_LOCKED));
			ChildRect.Set(mReal.wLeft + 74, mReal.wTop + 54, mReal.wLeft + 80, mReal.wTop + 62);
			pBitmap = &gb_Lock_Bitmap;
			break;
        case PLAYER_LOCKED_POWOFF:
			m_pString->DataSet(LS(SID_PLAYER_LOCKED));
			ChildRect.Set(mReal.wLeft + 74, mReal.wTop + 54, mReal.wLeft + 80, mReal.wTop + 62);
			pBitmap = &gb_Lock_Bitmap;
            break;
...
void
CSystemMessageScreen::SetScreenDataByChar(eSystemMessage eSysMesg, const char* pszMesg)
{
    TCHAR temp[strlen(pszMesg)+1];
    CharToTchar(temp,pszMesg);
    SetScreenData(eSysMesg,temp);
}
		...
		case PM_TIMER:
			...
			switch (Mesg.iData)
			{
				case TIMER_SCREEN_CHANGE:
					if(m_eSysMesg == LOW_POWER || m_eSysMesg == PLAYER_LOCKED_POWOFF)
					{
...
#ifdef __POGO
                        void (*pfnRestartAtZero)() = 0;
                        pfnRestartAtZero();
#else
						while(1);
#endif
					}
					else if(m_eSysMesg == USB_CONNECTED || m_eSysMesg == TEXT_MSG_NO_TIMEOUT)
					{
...
{
    CSystemMessageScreen* sms = CSystemMessageScreen::GetSystemMessageScreen();
