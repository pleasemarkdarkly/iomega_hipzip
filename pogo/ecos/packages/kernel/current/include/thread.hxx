...
public:    
    ...
    char                        *get_name();
		...
		// Track the CPU usage
protected:  // must be accessible to scheduler
    cyg_tick_count		cpu_start_time;  // time value for when we were put on the cpu
    cyg_tick_count		cpu_total_time;  // total ticks on the cpu

public:
    inline cyg_tick_count	get_cpu_usage()   { return cpu_total_time; }
    inline void             reset_cpu_usage();
