...
load_srec_image(int (*getc)(void), void (*terminate)(int method,int (*getc)(void)), unsigned long base)
{
    ...
    while ((c = (*getc)()) > 0) {
        ...
        case '3':
            ...
            offset += (type-'1'+2) * 2; // per grant edwards
            if (first_addr) {
                if (base) {
...
#endif
            count -= ((type-'1'+2)+1);
            offset += count * 2; // per grant edwards
            while (count-- > 0) {
                val = _hex2(getc, 1, &sum);
                *addr++ = val;
            }
            cksum = _hex2(getc, 1, 0);
            offset += 2; // per grant edwards
            sum = sum & 0xFF;
            cksum = (~cksum & 0xFF);
            if (cksum != sum) {
		if (terminate) (*terminate)(xyzModem_abort, getc);
        ...
        case '9':
            ...
            offset += ('9'-type+2) * 2; // per grant edwards
...
do_load(int argc, char *argv[])
{
    ...
    init_opts(&opts[3], 'm', true, OPTION_ARG_TYPE_STR, 
              (void **)&mode_str, (bool *)&mode_str_set, "download mode (TFTP, xyzMODEM, disk, or USB)");
    num_options = 4;
#if CYGNUM_HAL_VIRTUAL_VECTOR_NUM_CHANNELS > 1
...
#endif
#ifdef CYGPKG_REDBOOT_USB
        } else if (strcasecmp(mode_str, "usb") == 0) {
            mode = MODE_USB;
#endif
        } else {
            diag_printf("Invalid 'mode': %s\n", mode_str);
    ...
    if (raw) {
        ...
        if (0 == err) {
            diag_printf("Raw file loaded %p-%p, length=%p\n", (void *)base, (void *)end, ((void*)end-(void*)base));
        } else {
            diag_printf("Error in raw file load: %d, raw=%d\n", err, (raw?1:0) );
        }
    } else {
