        ...
        if ( attributes & ADIRENT )
        {
				...
				&crdate);
                        ...
                        pc_init_inode( lfinode,
                                ...
                                (TEXT *)string_3_spaces,
                                ADIRENT,
                                pc_sec2ClusterDir(pdrive, pobj->blkinfo.my_frstblock),
                                0L /*size*/,
                                &crdate);
		...
		while ( i < INOPBLOCK )
                {
					 ...
					 (UINT16 *)pbuff->data,
                        ...
                        if ( (pi->fattribute == CHICAGO_EXT) &&
                                (pi->fname[0] != PCDELETE) )
                                longFileNameEntry = YES; 

                        /* New entry in the directory entries if name is NULL */
                        if ( pi->fname[0] == 0 )
                        {
                                goto UPDATE_NAME_ENTRY;
                        }
                        else if ( pi->fname[0] == PCDELETE )
                        {
                                if ( longFileNameEntry )
                                {
                                        longFileNameEntry = NO;
...
{
#if !(LITTLE_ENDIAN) && (USE_HW_OPTION) && !(CHAR_16BIT)
        UINT16 i = 0;
        ULONG ii = 0L;
#endif
/*
...
SDVOID pc_ino2dos (DOSINODE *pbuff, FINODE *pdir) /*__fn__*/
{
	...
	pbuff->fcrttime = pdir->fcrttime;
	pbuff->fcrtdate = pdir->fcrtdate;
#endif
