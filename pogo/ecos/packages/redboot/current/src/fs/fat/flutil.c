...
SDBOOL pc_fileparse ( TEXT *filename, TEXT *fileext, TEXT *p ) /* __fn__*/
{
	...
	while ( *p ) 
	{
		...
		else
		{
			if ( i < 8 )
			{
				/* Since long file name allows space character */
                                if ( *p == ' ' )
				{
					i--;
					tmpPtr--;
                                        if ( !pc_allspace(p+1, (INT16)(namePat+7-p)) )
					{
						longName = YES;
					}
				}
...
/*        extPat = p;*/
	i = 0;
        ...
        while (extPat > p)
        {
                ...
                extPat--;
                i++;                    /* Counting number of char till . */
        }
        ...
        /* Now, save the extension name */
        while (i > 0)
        {       /* Long file name support "space" character */
                if (*extPat == ' ')
                {
                        i--;
                        tmpPtr--;
                        if ( !pc_allspace(extPat+1, (INT16)(i)) )
                        {
                                longName = YES;
			}
                }
	...
	while ( *p )
	{
		...
		{       /* Long file name support "space" character */
			if (*p == ' ')
			{
				i--;
                                tmpPtr--;
                                if ( !pc_allspace(p+1, (INT16)(extPat+2-p)) )
				{
					longName = YES;
				}
			}
