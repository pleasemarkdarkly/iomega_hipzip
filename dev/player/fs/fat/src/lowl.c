 ...
 *	 pc_free_clusters    -   Count free clusters in the FAT.
...
UCHAR *pc_FATRead( DDRIVE *pdr, UINT32 index, UINT32 starting_buffer_index, UINT16 count) /*__fn__*/
{
	...
	*/
    ...
    /* Not mapped, we have to read it in.
       ...
       until we get there.
    */
    DBASSERT( FAT_FS, !pdr->fat_is_dirty, "Reading in FAT on top of unflushed FAT\n");
    ...
    while ((starting_buffer_index + (UINT32)blk_counts) > (UINT32)ij )
    {
        ...
        DBASSERT( FAT_FS, pfs->pdirty[ij] == 0, "Reading in FAT on top of unflushed fat block %d\n", ij);
...
UCHAR *pc_pfswap(DDRIVE *pdr, UINT32 index) /*__fn__*/
{
    ...
    UCHAR   *pdata;
    ULONG   ltemp;
    UINT32  index_from, index_to, halfway, count;
    ...
    for (index_to = 0; index_to < FAT_BUFFER_SIZE; index_to++)
    {
        ...
        */
        if ( index_from == pfs->data_map[index_to])
        {
            ...
            // current_access is a very sketchy member - other routines rely on it referring to the most
            // recently accessed item in the fat table, not cool
    ...
    index_from = 0;
    count = 1; // default for single block fat cache
    if ( pfs->n_blocks_total > 1 )
    {

        // dc- these comments are helpful if you are a retarded dwarf from the land of shangrila. otherwise they have nothing to do
        //  with reality. what really happens here is if you are using the fat block at index 0, it swaps in everything else in the fat.
        //  otherwise, it swaps in from index 0 to the index you are using. this caching scheme is known as 'most frequently stoned and
        //  asking self what would philosophy majors do?'
        ...
        */
        ...
        pdata += ltemp;
#else
        // dc- instead of doing the above, do a simple 'flip' scheme. break the cache into two halves; whichever half 'current_access'
        //     is _not_ in gets swapped out
        halfway = FAT_BUFFER_SIZE >> 1;

        if( pfs->current_access >= (halfway) ) {
            // it's in the second half, so swap out the first
            index_from = 0;
        }
        else {
            // it's in the first half, so swap out the second
            index_from = halfway;
        }
        
        // Now that we know which half gets flipped out, verify we dont have any overlap. the trick here is that since
        // the cache is flipped out in halves, we can just check the start of the half that isn't getting flipped out ; if it falls
        // within the range of the block we are flipping in, then we have a problem. if it doesn't, then none of the other sectors in that
        // half can, since they are contiguous. if this does happen, we have to find the last block we can read in without having a
        // collision, then invalidate all entries following that block in our half, so as to preserve the contiguousness (<--mine)

        // make index_to the start of the half we are _not_ flipping in
        if( index_from == 0 ) index_to = halfway;
        else index_to = 0;

        // check to see if the other half has overlap
        if( pfs->data_map[ index_to ] >= (index>>7) && pfs->data_map[ index_to ] <= ((index>>7)+halfway) ) {
            // the first sector in the other half falls within the page we are flipping in
            
            // figure out how many sectors we can safely swap in
            count = pfs->data_map[ index_to ] - (index>>7);

            // abuse index_to here, since we can't change count
            index_to = count;
            
            // invalidate any other sectors in our half of the map (which is clean right now)
            while( index_to < (index_from + halfway) ) {
                // set this to something that will never generate a cache hit
                pfs->data_map[ index_to ] = 0xffffffff;
                index_to++;
            }
        }
        else {
    ...
    /*
      ...
      target for swapping.
    */
    if ( pc_FATRead( pdr, index, index_from, (UINT16)count ) == SDNULL )
        return SDNULL;
...
SDBOOL pc_updatedskfree(INT16 driveno) /*__fn__*/
{
    ...
    if (to_DWORD(b+0x01E4) == FSINFOSIG)
    {
            ...
            (to_DWORD(b+0x01EC) != (ULONG)FSFREEPTR)  )
        {
            ...
            // dc- write out the updated free_contig_pointer instead of FSFREEPTR
            fr_DWORD(&b[0x01EC], (ULONG)pdr->free_contig_base);
...
/*****************************************************************************
 * Name: pc_ifree_clusters - Count the number of free clusters remaining on a disk
 * (internal)
 *
 * Description:
 *       Given a drive number count the number of free clusters on the drive.
 *       Large disks can have more than 2^32 bytes free, but the number of
 *	free clusters on a FAT32 volume will fit in an unsigned long.
 *
 * Entries:
 *       INT16 driveno
 *	UINT16 *bytespcluster
 *
 * Returns:
 *       The number of free clusters or zero if the drive is full or 
 *       it it not open or out of range. If bytespcluster is nonzero,
 *	copies pdr->bytespcluster (for convenience).
 *
 * NOTE: To speed up this operation we maintain a variable in the
 *       drive structure.  known_free_clusters. If this variable is
 *       non zero a multiple of this value will be returned. To force
...
ULONG pc_ifree_clusters(INT16 driveno, UINT16 *bytespcluster) /*__fn__*/
{
	DDRIVE  *pdr;
    ULONG   ltemp;
    UINT32  nxt;
	...
	if (bytespcluster)
		*bytespcluster = pdr->bytespcluster;
...
INT16 get_partition_info(INT16 driveno) /*__fn__*/
{
	...
	BLKBUFF *bbuf;
    UCHAR   *buf;
#if (LITTLE_ENDIAN==0) && (USE_HW_OPTION==0)
	UINT16  *pbuf;
#endif
    ULONG   ltemp;
    INT16   nparts;
	...
	// attempt to determine - is this a partition table or a 
	// partitionless fat volume

	// temancl notes: - this is not an ideal detection scheme,
	// but signature testing is not appropriate;
...
#if LITTLE_ENDIAN
#else
#error big endian partition code likely to be broken
#endif

		// fat boot sector (jump instruction)
		BOOTSECTCOMMON bscom;
		BOOTSECT16 bs16;
		BOOTSECT32 bs32;
		...
		{
			diag_printf("error: invalid signature 0x%2.2x 0x%2.2x\n",buf[510],buf[511]);			
		}
		else
		{

			memcpy((void*)&bscom,(void*)buf,sizeof(BOOTSECTCOMMON));

			if(bscom.bsRootEntries == 0)
			{

				// FAT32 system
				memcpy((void*)&bs32,(void*)(buf+36),sizeof(BOOTSECT32));
				
				if(bs32.bsBootSignature == 0x29)
				{
					pdr->os_type = (UTINY)WIN98_TYPE;
					pdr->volume_start = 0;
					pdr->volume_end = bscom.bsBigTotalSectors;
					pdr->enable_mapping = YES;
					nparts = 1;		
				}
				else
				{
					diag_printf("error: invalid fat32 boot sig 0x%2.2x\n",bs16.bsBootSignature);
				}
			...
			{
				// FAT16 system
				memcpy((void*)&bs16,(void*)(buf+36),sizeof(BOOTSECT16));

				if(bs16.bsBootSignature == 0x29)
				{
					pdr->os_type = (UTINY)DOS_TYPE;
					pdr->volume_start = 0;
					
					if(bscom.bsBigTotalSectors == 0)
						pdr->volume_end = bscom.bsTotalSectors;
					else
						pdr->volume_end = bscom.bsBigTotalSectors;
				...
				{
					diag_printf("error: invalid fat16 boot sig 0x%2.2x\n",bs16.bsBootSignature);
				}
	...
	else // it is a partition table, or totally invalid	
	{
		
		diag_printf("warning: attempting partition read\n");
		// partition table - unpack and retrieve first partition,
		// then retrieve 

		// Word align this, always
