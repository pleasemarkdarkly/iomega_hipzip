...
INT16 platform_convert_critical_error(INT16 error_code) /*__fn__*/
{
	...
	switch (error_code) {
	    ...
	    default:
		DEBUG(FAT,DBGLEV_INFO,"unhandled error code %x\n", error_code);
...
GetFatErrorString(int error)
{
    ...
    switch (error)
    {
        ...
        case PENOENT:
            return "File not found or path to file not found";
        case PEMFILE:
            return "No file descriptors available (too many files open)";
        case PEEXIST:
            return "Exclusive access requested but file already exists.";
        case PEACCES:
            return "Attempt to open a read only file or a special (directory)";
        case PEINVAL:
            return "Seek to negative file pointer attempted.";
        case PENOSPC:
            return "Write failed. Presumably because of no space";
        case PESHARE:
            return "Open failed do to sharing";
        case PEDEVICE:
            return "No Valid Disk Present";
        case 0:
            return "No Error";
        default:
