// dp.c: routines to interace with the dataplay drive
...
int dp_get_device_info( dp_device_info_t* ret ) {
	...
	if( dp_verify_phase( DP_READ_PHASE ) ) {
		dp_read_response( (char*)ret, sizeof( dp_device_info_t ) );
	}
...
int dp_lock_media( void ) {
	...
	cmd_packet[0] = DPICMD_LOCKREL_MEDIA;
...
int dp_get_attention_info( dp_attention_info_t* ret) {
	...
	dp_send_cmd( &cmd_packet, 1 );
	if( dp_verify_phase( DP_READ_PHASE ) ) {
		dp_read_response( (char*)ret, sizeof( dp_attention_info_t ) );
	}
...
int dp_load_media( void ) {
	...
	cmd_packet[0] = DPICMD_LOADEJECT_MEDIA;
	cmd_packet[1] = 0x80;  // load media
