...
char* comment = NULL; /* comment string */
int comment_len = 0;  /* comment string length */
int header_comment = 0; /* no comment by default */
...
struct option longopts[] =
{
 /* { name  has_arg  *flag  val } */
    ...
    {"comment",    1, 0, 'C'}, /* save comment into gzip */
...
local void usage()
{
    printf ("usage: %s [-%scCdfhlLnN%stvV19] [-S suffix] [file ...]\n",
	    progname,
#if O_BINARY
	    "a",
#else
...
local void help()
{
 ...
 " -C text --comment text    save string in comment field",
    ...
    char **argv;
{
    ...
    while ((optc = getopt_long (argc, argv, "ab:cC:dfhH?lLmMnNqrS:tvVZ123456789",
				longopts, (int *)0)) != -1) {
	switch (optc) {
        case 'a':
            ascii = 1; break;
