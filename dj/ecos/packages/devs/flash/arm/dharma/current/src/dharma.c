...
#define BLOCKSIZE (0x10000)
        ...
        // Gay hack to get around issues with wiring on integrex v2; force the size of the chips
        // to a fixed value
        //        num_regions = _si(qp->num_regions)+1;
        region_size = _si(qp->region_size)*256;       
        num_regions = (CYGNUM_FLASH_SIZE) / (region_size * CYGNUM_FLASH_DEVICES);
	...
	diag_printf("Flash Found -  %x, dev %x, id [%4s] stat %x\n blocksize %x buffersize %x blocks %x start %x end %x\n",
           qp->manuf_code, qp->device_code, qp->id, stat,flash_info.block_size,
 ...
 flash_type_unknown:
    diag_printf("Can't identify FLASH, sorry, man %x, dev %x, id [%4s] stat %x\n",
