...
static void cs8900a_TransmitFrame( struct eth_drv_sc *sc, struct eth_drv_sg* sg_list, int sg_len );
...
cs8900a_init(struct cyg_netdevtab_entry *tab)
{
...
#ifdef CYGSEM_DEVS_ETH_CL_CS8900A_CHIP_TEST
    {
        int write_val, read_val;
        int reg;
        // Perform basic on-chip memory tests
        // Walk 1s on data bus
        for (write_val = 0x0001; write_val != 0; write_val <<= 1) {
            for (reg = 0x0150; reg <= 0x015d; reg += 2) {
                put_reg(base, reg, write_val);
                read_val = get_reg(base, reg);
                if (write_val != read_val) {
                    diag_printf("CS8900a - On chip memory test (data bus) failed at 0x%04x\n", reg);
                    return false;
                }
            }
        }
        ...
        for (reg = 0x0150, write_val = 0x0101; reg <= 0x015d; reg += 2, write_val += 0x0101) {
            read_val = get_reg(base, reg);
            if (write_val != read_val) {
                diag_printf("CS8900a - On chip memory test (incrementing value) failed at 0x%04x\n", reg);
                return false;
            }
        }
    }
    ...
    // Wait for EEPROM busy to clear
    if( get_reg(base, PP_SelfStat) & PP_SelfStat_EEPROM ) {
        while ((get_reg(base, PP_SelfStat) & PP_SelfStat_SIBSY) == 0) {
            if (--timeout <= 0) {
#if DEBUG & 8
                diag_printf("CS8900 EEPROM didn't reset - abort!\n");
#endif
                return false;
            }
        }
...
cs8900a_stop(struct eth_drv_sc *sc)
{
    ...
    while( (get_reg(base, PP_SelfStat) & PP_SelfStat_InitD) == 0) {
        if( --timeout <= 0 ) {
            diag_printf("CS8900a - device timed out while waiting for disable\n");
            break;
        }
    }
}
...
cs8900a_start(struct eth_drv_sc *sc, cyg_uint8 *esa, int flags)
{
    ...
    ctl_word = PP_RxCTL_RxOK | PP_RxCTL_Broadcast | PP_RxCTL_IA;

#ifdef CYGSEM_DEVS_ETH_CL_CS8900A_SUPPORT_MULTICAST
    ctl_word |= PP_RxCTL_Multicast;
#endif
    
    put_reg(base, PP_RxCTL, ctl_word );
...
cs8900a_control(struct eth_drv_sc *sc, unsigned long key, void *data, int data_length)
{
    switch (key) {
        ...
        case ETH_DRV_GET_IF_STATS:
        {
            ...
            if( !cpd->link_up ) {
                // if the link is down, test link status again
                unsigned short stat = get_reg( cpd->base, PP_LineStat );
                if( (stat & PP_LineStat_LinkOK) == 0 ) {
                    p->operational = 2; // LINK DOWN
                }
                else {
                    p->operational = 3; // LINK_UP
                    cpd->link_up = true;
                }
            }
            return 0;
            ...
            int total_len, unsigned long key)
{
...
#ifdef CYGPKG_NET
    // Start only when partial frame already sent
    HAL_WRITE_UINT16(cpd->base+CS8900A_TxCMD, PP_TxCmd_TxStart_381);
#else
    ...
    cpd->sg_list = sg_list;
    cpd->sg_len = sg_len;
    if( (stat = get_reg(base, PP_BusStat)) & PP_BusStat_TxRDY ) {
        cs8900a_TransmitFrame( sc, sg_list, sg_len );
    }
#else
    do {
...
static void
cs8900a_TransmitFrame( struct eth_drv_sc *sc, struct eth_drv_sg* sg_list, int sg_len )
{
    cs8900a_priv_data_t *cpd = (cs8900a_priv_data_t *)sc->driver_private;
    cyg_addrword_t base = cpd->base;
    int i, len;
    ...
    for( i = 0; i < sg_len; i++ ) {
        ...
        if( len > 0 ) {
            ...
            if (odd_byte) {
                saved_data |= ((cyg_uint16)*data++) << 8;
                HAL_WRITE_UINT16(base+CS8900A_RTDATA, saved_data);
                len--;
                odd_byte = false;
            }
            /* Output contiguous words. */
            sdata = (cyg_uint16 *)data;
            while (len > 1) {
                HAL_WRITE_UINT16(base+CS8900A_RTDATA, *sdata++);
                len -= sizeof(cyg_uint16);
            }
            /* Save last byte, if necessary. */
            if (len == 1) {
...
cs8900a_BufEvent(struct eth_drv_sc *sc, int stat)
{
    ...
    if (stat & PP_BufCFG_TxRDY) {
        if( cpd->txbusy ) {
            cs8900a_TransmitFrame( sc, cpd->sg_list, cpd->sg_len );
        }
    }
    if (stat & PP_BufCFG_RxMiss) {
#if DEBUG & 8
        diag_printf("CS8900a - RxMiss, stat 0x%x\n", stat);
#endif
    }
    if (stat & PP_BufCFG_TxUE) {
#if DEBUG & 8
        diag_printf("CS8900a - buffer underrun, stat 0x%x\n", stat);
#endif
