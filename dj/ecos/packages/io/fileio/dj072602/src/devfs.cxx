                          ...
                          int mode,  cyg_file *file )
{
    ...
    // Determine the type of the file based on the device type
    // This may or may not be a good idea
    if( ((cyg_devtab_entry_t*)handle)->status & CYG_DEVTAB_STATUS_BLOCK ) {
        file->f_type = CYG_FILE_TYPE_BLK;
    }
    else {
        file->f_type = CYG_FILE_TYPE_CHR;
    }
...
if(whence == 0) // only support absolute seeks for the moment
    fp->f_offset = *pos;
else
    *pos = 0;
