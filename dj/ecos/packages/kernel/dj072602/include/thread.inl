...
inline void Cyg_Thread::reset_cpu_usage()
{
    cpu_total_time = 0;
    // conditionally reset the cpu_start_time, otherwise
    // the current thread looks like it has spent a looooong time on the cpu
    if( Cyg_Thread::self() != this ) {
        cpu_start_time = 0;
    }
}
