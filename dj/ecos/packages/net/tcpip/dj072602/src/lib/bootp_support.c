...
init_net(const char *intf, struct bootp *bp)
{
    ...
    length = sizeof(addrp->sin_addr);
    if( get_bootp_option(bp, TAG_GATEWAY, &gateway,&length) && 0 != gateway.s_addr ) {
    }
    else {
        // set ourselves as a local gateway if none is provided for us
        gateway = bp->bp_yiaddr;
    }
    memset(&route, 0, sizeof(route));
    addrp->sin_family = AF_INET;
    addrp->sin_port = 0;
...
#endif // CYGINT_ISO_DNS
    
#ifdef CYGPKG_NET_DNSCLIENT
    {
        struct in_addr dns;
        length = sizeof( struct in_addr );
        if( get_bootp_option( bp, TAG_DOMAIN_SERVER, &dns, &length) ) {
            if( 0 != dns.s_addr ) {
                dns_use_server( &dns );
            }
        }
