...
static void listdir( const char *name, int statp )
{
    int err;
    DIR *dirp;
    ...
    for(;;)
    {
        ...
        if( entry == NULL ) {
            diag_printf("<INFO>: dir gave back null entry ptr\n");
            break;
        }
...
#include <cyg/kernel/kapi.h>
static cyg_handle_t threadh;
static cyg_thread   thread;
static char         tstack[65536] __attribute__((aligned(4)));;

void thread_entry( cyg_uint32 foo );
int main( int argc, char **argv )
{
    cyg_thread_create( 10, thread_entry, (unsigned int)0, "thr",
        (void*)tstack, 65536, &threadh, &thread);
    cyg_thread_resume( threadh );
...
void thread_entry( cyg_uint32 foo )
{
    ...
    // first mount devfs, this is required for filesystems to mount on
    // block devices properly
    err = mount("", "/dev", "devfs");
    if( err < 0 ) SHOW_RESULT( mount, err );
    
    do {
        err = mount("/dev/hda/", "/hd", "fat");
        if (err < 0) SHOW_RESULT(mount, err);
    } while (err < 0);
...
#if 0
    fd = open("/hd/READ1ST.TXT", O_RDONLY);
    if (fd < 0) {
	    SHOW_RESULT(open, fd);
    }
    else {
    ...
    // Open for writing 
    fd = open("/hd/READ1ST.TXT", O_WRONLY);
    if (fd < 0) {
	    SHOW_RESULT(open, fd);
    }
    else {
