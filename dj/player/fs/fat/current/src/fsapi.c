 ...
 *
 ******************************************************************************/
SDBOOL pc_mv(TEXT *sourcepath, TEXT *targetpath) /*__fn__*/
{

    DROBJ *sourcedir = NULL;
    DROBJ *targetdir = NULL;
    DROBJ *sourcefile = NULL;
    DROBJ *targetfile = NULL;
    ...
    // temporary path pointer
    TEXT    *temppath;
    
    // 8.3 storage
    TEXT    sourcefileext[4];
    TEXT    sourcefilename[10];
    TEXT    targetfileext[4];
    TEXT    targetfilename[10];


    // backup important source info
    DOSINODE sourcenode;
    ...
    // Get the drive and make sure it is mounted */
    driveno = check_drive(sourcepath);
    if ( driveno < 0 )
    {
        PC_FS_EXIT();
        return(NO);
    ...
    temppath = (TEXT *)fspath;

    ////////////
    // get source path
    temppath[0] = 0;
    if ( !validate_current_information( temppath, sourcefilename, sourcefileext, sourcepath ) )
        goto mverr;    
    longFileName = saveFileName;
    ////////////
    ...
    sourcedir = pc_fndnode(temppath);

    if ( !sourcedir )
        goto mverr;

    /* lock the directory */
    PC_INODE_ENTER(sourcedir->finode, YES);
    ...
    // verify source file
    sourcefile = pc_get_inode(SDNULL,
        sourcedir,
        sourcefilename,
        sourcefileext);
    ...
    // get target path
    temppath[0] = 0;
    if ( !validate_current_information( temppath, targetfilename, targetfileext, targetpath ) )
        goto mverr;    
    longFileName = saveFileName;
    /////////////////
    ...
    targetdir = pc_fndnode(temppath);

    if ( !targetdir )
        goto mverr;

    /* lock the directory */
    PC_INODE_ENTER(targetdir->finode, YES);
    ...
    // verify target file
    targetfile = pc_get_inode(SDNULL,
        targetdir,
        targetfilename,
        targetfileext);
    ...
    //
    // store and remove old directory entry (saves clusters, time, size, etc)
    //

    copybuff(&sourcenode, sourcefile->finode, sizeof(DOSINODE));

    sourcefile->finode->fname[0] = PCDELETE;
    ...
    // remove directory entry from source directory, but DO NOT UNLINK CLUSTERS!!!!    
    if (!pc_update_inode(sourcefile, YES, YES) )
    {
        p_errno = PEACCES;
        goto mverr;
    }
    ...
    // create a new node in target directory
    targetfile = pc_mknode( targetdir,
        targetfilename,
        targetfileext,
        &sourcenode,
        (UINT16)sourcenode.fattribute,
        1); // copy some old info from sourcenode

    

    // re-associate old cluster chain, file name, etc, the cheap way
    copybuff(targetfile->finode, &sourcenode, sizeof(DOSINODE));
    copybuff(targetfile->finode->fname, targetfilename, (8 * sizeof(UTEXT)) );
    copybuff(targetfile->finode->fext,  targetfileext, (3 * sizeof(UTEXT)) );

    // update
    if ( !pc_update_inode(targetfile, YES, YES) )
    {
        p_errno = PENOSPC;
        goto mverr;
    }
    ...
    if(sourcedir)
    {
        PC_INODE_EXIT(sourcedir->finode);
        pc_freeobj(sourcedir);
    }

    if(targetdir)
    {
        PC_INODE_EXIT(targetdir->finode);
        pc_freeobj(targetdir);
    }

    if(sourcefile)
    {
        pc_freeobj(sourcefile);
    }

    if(targetfile)
    {
        pc_freeobj(targetfile);
    }
...
// critical bugs, so many things wrong here
static SDBOOL pc_mv_broken(TEXT *name, TEXT *newname) /*__fn__*/
{
    DROBJ   *pobj;
    DROBJ   *parent_obj;
    BLOCKT  save_myblock;
    ...
    /* verify the path for the old file */
    path = (TEXT *)fspath;
    path[0] = 0;
    if ( !validate_current_information( path, filename, fileext, name ) )
        goto errex;
    ...
    // temancl - why is this done?
    longFileName = saveFileName;

    /* try and get the entry for the new path in the parent */
    pobj = pc_get_inode(SDNULL,
        parent_obj,
        newfilename,
        newfileext);
    ...
    /* get the entry for the old filename */
    pobj = pc_get_inode(SDNULL,
        parent_obj,
        filename,
        fileext);
    ...
    if ( !isLongFileName ) // if renaming to short
    {
        if (currFileName[0] != 0)
        {
            ...
            this block of code does something bad
            pc_update_node apparently has issues moving between long and short filenames
            lfn entry delete should be:
            mark fname[0] as PCDELETE
            find associated long entries?
    ...
    // is the new entry a long filename? this works OK
    if ( isLongFileName )
    {
        path[0] = 0;
        ...
        // overwrite important bits of fnode, so it points to the right clusters 
        // AND has the right file name
        ...
        if ( !pc_update_inode(pobj, YES, NO) )  // dc- was YES, YES
        {

            // temancl is confused. why do this?
...
#if 0
/* broken implementation, use pc_free64() */
/******************************************************************************
...
UINT64 pc_free64(INT16 driveno)
{
    UINT64 ret_val = 0;
    ...
    if ( check_media_entry(driveno) )
        ret_val = pc_ifree_clusters(driveno,&bytespcluster);
...
SDBOOL pc_gfirst(DSTAT *fsstatobj, TEXT *name) /*__fn__*/
{
    ...
    saveFileName = SDNULL;
    statobj = (FSDSTAT *)&(fsstatobj->statobject);
...
SDBOOL pc_set_default_drive( INT16 driveno ) /*__fn__*/
{
    ...
    if ( driveno >= 0 )
    {
        // pc_setdfltdrvno locks the FS mutex
        ret = (pc_setdfltdrvno(driveno));
    }
    else
        ret = (NO);
...
{
    // read-only so this is technically safe
    CHECK_MEM(COUNT, 0);     /* Make sure memory is initted */
    return(fs_user->dfltdrv);
}


static SDBOOL pc_inside_subdirectory (DROBJ *pobj) /*__fn__*/
{
    DROBJ *pcwd;
    UINT32 clusterno;
    UINT32 clusternum;
