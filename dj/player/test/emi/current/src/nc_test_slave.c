...
#define DESIRED_BACKGROUND_LOAD  0
...
void NETUIWait()
{
	cyg_mutex_lock(&netuim);
	cyg_cond_wait(&netui);
	cyg_mutex_unlock(&netuim);
}
...
static void net_static_init(char *szip)
{
...
net_test(int param)
{
		...
		if(param == 0)
			...
			net_static_init(NC_SLAVE_STATIC_IP);

		test_printf("init done - start slave test\n");

		nc_slave(0);
		show_net_times(); 
		
		test_printf("test complete\n");
...
void
net_thread(cyg_uint32 ignored)
{

	cyg_mutex_init(&netuim);
	cyg_mutex_lock(&netuim);
	cyg_cond_init(&netui, &netuim);
	cyg_mutex_unlock(&netuim);

	// test_printf("dhcp - FIXME for static?\n");
	net_test(1);
}
