...
nc_slave(test_param_t param)
{
    ...
    while (!done) {
		...
		tv.tv_sec=0;
		tv.tv_usec=100000; // 100us timeout

		setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)); 
		while(recvfrom(s, &req, sizeof(req), 0, (struct sockaddr *)&master, &masterlen) < 0)
		{
			if(isbreak())
			{
				close(s);
...
net_test(int param)
{
		...
		printf("init done - start slave test\n");
