...
do_udp_test(int s1, int type, struct sockaddr_in *slave,
            int nbufs, int buflen, int pause_time, int pause_threshold, int *results)
{
    int i, s, td_len, seq, seq_errors, total_packets, ret;
...
//                                    ntohl(tdp->seq), seq);
    ...
    ret = show_results(type_name, &start_time, &end_time, total_packets, buflen, 
...
//                                    ntohl(tdp->seq), seq);
    ...
    gettimeofday(&end_time, 0);
    res = show_results(type_name, &start_time, &end_time, total_packets, buflen, 
                 lost_packets, seq_errors);
    // Fetch results record
...
nc_master(struct test_params *p)
{
    ...
    p->isend = do_tcp_test(s, NC_REQUEST_TCP_SEND, &slave, 640, 1024, 0, 0);
    p->irecv = do_tcp_test(s, NC_REQUEST_TCP_RECV, &slave, 640, 1024, 0, 0);
...
int WINAPI test_dll(char* ip, int* isend, int* irecv)
{
