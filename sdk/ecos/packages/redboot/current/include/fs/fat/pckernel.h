...
extern cyg_mutex_t fs_mutex;  // defined in fs/fat/pckernel.c
#define PC_FS_ENTER()
#define PC_FS_EXIT()
#define CHECK_USER(x,y)
#define VOID_CHECK_USER()
...
#if 0
#define PC_SYNC_INIT()       diag_printf("mutex_init\n"); cyg_mutex_init( &fs_mutex );
#define PC_ENTER_CRITICAL()  diag_printf("%s %d mutex_lock\n",__FUNCTION__,__LINE__); cyg_mutex_lock( &fs_mutex );
#define PC_EXIT_CRITICAL()   diag_printf("%s %d mutex_unlock\n",__FUNCTION__,__LINE__); cyg_mutex_unlock( &fs_mutex );
#else
#define PC_SYNC_INIT()       cyg_mutex_init(   &fs_mutex );
#define PC_ENTER_CRITICAL()  cyg_mutex_lock(   &fs_mutex );
#define PC_EXIT_CRITICAL()   cyg_mutex_unlock( &fs_mutex );
#endif
