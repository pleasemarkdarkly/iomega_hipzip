...
/*****************************************************************************
* Name: wcopyLongFileName
*
* Description:
*       Calculate the name length and copy the name using double byte charactes.
*
* Entries:
*       INT16 offset
*       INT16 index
...
{
	INT16 i, iPos, strLength;
	SDWCHAR tmpChar;
	
	iPos = (offset * LFNLEN_PER_ENTRY) + displ;
	for( i = 0, strLength = 0; i < index; i++ )
	{
		tmpChar  = (SDWCHAR)(*((UTEXT *)curNameExt + (i << 1)));
		tmpChar |= (SDWCHAR)(*((UTEXT *)curNameExt + (i << 1) + 1) << 8);
		if( (tmpChar == 0x0000) || (tmpChar == 0xffff) )
			break;
		
		*(SDWCHAR *)(fNameExt + iPos + i ) = tmpChar;
		strLength += 1;
	}
	return (strLength);
}
