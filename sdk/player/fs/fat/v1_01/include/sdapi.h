...
/* OEM platform specific */
  //#include "oem.h"
    ...
    // dc- use standard C routines so we get sanity checking and assertions
#include <string.h>
#define copybuff memcpy
#define compbuff memcmp
#define pc_memfill( a, b, c)  memset(a, c, b)
#define pc_strcat strcat
...
const char* GetFatErrorString(int error);
...
typedef struct fsdstat {
    struct dstat   *fileinfo;
    struct drobj   *pobj;          /* Info for getting at the inode */
    struct drobj   *pmom;          /* Info for getting at parent inode */
} FSDSTAT;
...
// dc- assume these are defined in groups
#ifndef S_ISDIR
...
#define UNSAFE_OPEN // dc- not really unsafe
#ifdef UNSAFE_OPEN
/* File access flags */
...
#else
// OR'd with NOSHAREWRITE
#define PO_RDONLY       0x0800 /* Open for read only */
// OR'd with NOSHAREANY
#define PO_WRONLY       0x0005 /* Open for write only */
#define PO_RDWR         0x0006 /* Read/write access allowed. */
#endif
...
*       Drive Cluster size
*****************************************************************************/
COUNT pc_cluster_size(INT16 driveno);
...
* Name: pc_free
*
* Processing:   Get available disk space. 64-bit variant needed for > 2GB drives
*		
*
* Entries:
*       INT16   driveno         Driver number
...
*       File Handle       
*****************************************************************************/
PCFD po_open(const TEXT *name, UINT16 flag, UINT16 mode);
...
*****************************************************************************/
ULONG po_read(PCFD fd,  UCHAR *buf, ULONG count);
...
*****************************************************************************/
ULONG po_write(PCFD fd, UCHAR *buf, ULONG count);
...
*                   
******************************************************************************/
UCOUNT po_extend_file(PCFD fd, UCOUNT n_clusters, COUNT method, SDBOOL preerase_region);
