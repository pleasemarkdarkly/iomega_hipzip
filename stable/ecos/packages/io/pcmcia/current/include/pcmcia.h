...
struct cf_irq_handler {
    void         (*handler)(cyg_vector_t, cyg_ucount32, cyg_addrword_t);
    cyg_addrword_t param;
};

#if defined(CYGIMP_KERNEL_INTERRUPTS_CHAIN)
struct cf_irq_isr_handler {
    int         (*handler)(cyg_vector_t, cyg_addrword_t);
    cyg_addrword_t param;
};
#endif
...
#if defined(CYGIMP_KERNEL_INTERRUPTS_CHAIN)
    struct cf_irq_isr_handler irq_isr_handler;
#endif
};
...
void cf_register_handler(struct cf_slot *, void (*handler)(cyg_vector_t, cyg_ucount32, cyg_addrword_t), cyg_addrword_t);
#if defined(CYGIMP_KERNEL_INTERRUPTS_CHAIN)
void cf_register_handler_isr(struct cf_slot *, int (*handler)(cyg_vector_t, cyg_addrword_t), cyg_addrword_t);
#endif
