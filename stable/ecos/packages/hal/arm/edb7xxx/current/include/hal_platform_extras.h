...
_PT_0x0:
	...
	// if DRAM size is zero, assume it is actually SRAM and map it in
	// we are using static tables, the extra space used by mapping in
	// too much memory is no biggie.
...
#err Static MMU tables not yet available for 2M boards.
        ...
        .set    PTE,(DRAM_PA_START+LCD_BUFFER_SIZE)|MMU_L2_TYPE_Small\
                    |MMU_AP_Any|MMU_Bufferable|MMU_Cacheable
        .rept   (0x1000000-LCD_BUFFER_SIZE)/0x1000
        NEXT_PAGE
        .endr
#if 0
        ...
        .endr
#endif
#elif (CYGHWR_HAL_ARM_EDB7XXX_DRAM_SIZE == 32)
        .set    PTE,(DRAM_PA_START+LCD_BUFFER_SIZE)|MMU_L2_TYPE_Small\
                    |MMU_AP_Any|MMU_Bufferable|MMU_Cacheable
        .rept   (0x20000000-LCD_BUFFER_SIZE)/0x1000
        NEXT_PAGE
        .endr
#else
#error "How much DRAM ?"
#endif // CYGHWR_HAL_ARM_EDB7XXX_DRAM_SIZE
        .balign 0x0400
...
#define FILL_16M_ROM_SECTION(base,seg)          \
        .long   base+(MMU_SECTION_SIZE*(seg+0x00));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x01));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x02));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x03));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x04));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x05));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x06));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x07));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x08));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x09));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x0A));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x0B));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x0C));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x0D));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x0E));   \
        .long   base+(MMU_SECTION_SIZE*(seg+0x0F));   \
	
#define FILL_256M_ROM_SEGMENT(base)             \
        FILL_16M_ROM_SECTION(base,0x00);        \
        FILL_16M_ROM_SECTION(base,0x10);        \
        FILL_16M_ROM_SECTION(base,0x20);        \
        FILL_16M_ROM_SECTION(base,0x30);        \
        FILL_16M_ROM_SECTION(base,0x40);        \
        FILL_16M_ROM_SECTION(base,0x50);        \
        FILL_16M_ROM_SECTION(base,0x60);        \
        FILL_16M_ROM_SECTION(base,0x70);        \
        FILL_16M_ROM_SECTION(base,0x80);        \
        FILL_16M_ROM_SECTION(base,0x90);        \
        FILL_16M_ROM_SECTION(base,0xA0);        \
        FILL_16M_ROM_SECTION(base,0xB0);        \
        FILL_16M_ROM_SECTION(base,0xC0);        \
        FILL_16M_ROM_SECTION(base,0xD0);        \
        FILL_16M_ROM_SECTION(base,0xE0);        \
        FILL_16M_ROM_SECTION(base,0xF0);
...
_MMU_table:     
        ...
        FILL_256M_SEGMENT(PCMCIA0_PA|MMU_L1_TYPE_Section|MMU_AP_Any)
        FILL_256M_SEGMENT(PCMCIA1_PA|MMU_L1_TYPE_Section|MMU_AP_Any)
        ...
        FILL_256M_SEGMENT(MMU_L1_TYPE_Fault)
        FILL_256M_ROM_SEGMENT(ROM0_PA|MMU_L1_TYPE_Section|MMU_AP_Any|MMU_Cacheable)
        FILL_256M_ROM_SEGMENT(ROM1_PA|MMU_L1_TYPE_Section|MMU_AP_Any|MMU_Cacheable)
