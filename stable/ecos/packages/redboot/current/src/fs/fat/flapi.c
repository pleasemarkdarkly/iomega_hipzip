...
******************************************************************************/
PCFD po_open(TEXT *name, UINT16 flag, UINT16 mode) /*__fn__*/
{
	PC_FILE *pfile;
	DROBJ   *parent_obj;
	DROBJ   *pobj;
...
*
******************************************************************************/
UCOUNT po_read(PCFD fd, UCHAR *pbuf, UCOUNT count) /*__fn__*/
{
	PC_FILE *pfile;
        UCHAR   *buf;
	DDRIVE  *pdrive;
	...
	while ( n_left )
	{
		...
		else
		{
			...
			n_to_read = (UCOUNT) (n_to_read - block_in_cluster);
...
*
******************************************************************************/
UCOUNT po_write(PCFD fd, UCHAR *buf, UCOUNT count) /*__fn__*/
{
	PC_FILE *pfile;
/*
        PC_FILE *tmp_pfile;
*/
	...
	*/
	...
	while ( n_left )
	{
		...
		{
			/* Extending the file */
			n_blocks_left = (UINT16)((n_left + 511) >> 9);
			...
			*/
			...
			if ( !pfile->pobj->finode->fclusterHi )
		...
		if ( n_clusters && (n_left > 511) )
		{
			...
			block_in_cluster = (UINT16) (pfile->fptr &
					pdrive->byte_into_cl_mask);
			block_in_cluster >>= 9;
			block_to_write = pfile->fptr_block + block_in_cluster;

			/* how many do we write ? */
			n_blocks_left = (UINT16)(n_left >> 9);
...
*
******************************************************************************/
ULONG _po_lseek(PC_FILE *pfile, INT32 offset, INT16 origin, INT16 *err_flag) /*__fn__*/
{
	DDRIVE  *pdrive;
	LONG    file_pointer;
	LONG    ret_val;
