...
/*----------------------------------------------------------------------*/
/* Write zero terminated wide string                                    */
  
externC void diag_write_wide_string(const unsigned short *psz)
{
    while( *psz ) diag_write_char( (char)*psz++ );
}
...
static cyg_bool diag_check_string( const char *str )
{
    ...
    for( s = str ; result && *s ; s++ )
    {
        ...
        if( s-str > MAX_STRING_LENGTH ) result = false;
...
/*----------------------------------------------------------------------*/
/* perform some simple sanity checks on a wide string to ensure that it */
/* consists of printable characters and is of reasonable length.        */

static cyg_bool diag_check_wide_string( const unsigned short *str )
{
    cyg_bool result = true;
...
_vprintf(void (*putc)(char c, void **param), void **param, const char *fmt, va_list ap)
{
    ...
    while ((c = *fmt++) != '\0') {
		...
		islong = 1;
            ...
            case 'w':
            case 'W':
                pwch = va_arg(ap, unsigned short * /* wchar */);
                if (pwch == NULL)
                {
                    cp = "<null>";
                    length = 6;
                ...
                else if (!diag_check_wide_string(pwch)) {
                    diag_write_string("<Not a string: 0x");
                    diag_write_hex((cyg_uint32)pwch);
                    cp = ">";
                    length = 1;
                    pwch = 0;
                }
