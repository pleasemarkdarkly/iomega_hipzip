	...
	for (; pcomp < pcompe;
		...
		switch (*pcomp->cflag) {
			...
			case ISO_SUSP_CFLAG_VOLROOT:
				...
				outbuf -= len;
				len = 0;
				inbuf = (char *)ana->imp->im_mountp->name;
				wlen = strlen(inbuf);
				break;
			...
			case 0:
				...
				break;
			default:
				diag_printf("RRIP with incorrect flags?");
				wlen = ana->maxlen + 1;
				break;
		}
		    ...
		    ISO_RRIP_ANALYZE *ana)
{
	...
	switch (*p->flags) {
		...
		default:
			diag_printf("RRIP with incorrect NM flags?\n");
			wlen = ana->maxlen + 1;
			break;
	}
 ...
 */
int
cd9660_rrip_offset(struct iso_directory_record *isodir,
		   struct iso_mnt *imp)
{
	ISO_RRIP_OFFSET *p;
	ISO_RRIP_ANALYZE analyze;
