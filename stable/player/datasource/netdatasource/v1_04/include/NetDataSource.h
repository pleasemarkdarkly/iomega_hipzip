...
#ifndef NETDATASOURCE_H_
#define NETDATASOURCE_H_

#include <datasource/common/DataSource.h>

#define NET_DATA_SOURCE_CLASS_ID  3

// fdecl
class IContentManager;
class IInputStream;
class IMediaContentRecord;
class CNetDataSourceImp;
...
class CNetDataSource : public IDataSource
{
public:
    
    static CNetDataSource* Open( int iInterface = 0, bool iForceInit = false );
    ...
    IMediaContentRecord* GenerateEntry(IContentManager* pContentManager, const char* pURL, int iCodecID = 0) const;
...
private:
    ...
    CNetDataSourceImp*  m_pImp;

    CNetDataSource(int iInterface, bool iForceInit);
    void SetInstanceID(int iDataSourceID);
};


#endif  // NETDATASOURCE_H_
