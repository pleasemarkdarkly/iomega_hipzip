// format.cpp --
// $Id: format.cpp,v 1.30 2002/04/28 10:58:24 wcvs Exp $
...
class c4_FormatW : public c4_FormatB
{
public:
    c4_FormatW (const c4_Property& prop_, c4_HandlerSeq& seq_);
...
c4_FormatW::c4_FormatW (const c4_Property& prop_, c4_HandlerSeq& seq_)
    : c4_FormatB (prop_, seq_)
{
}

int c4_FormatW::ItemSize(int index_)
{
    int n = c4_FormatB::ItemSize(index_) - 1;
    return n >= 0 ? n : 0;
}

const void* c4_FormatW::Get(int index_, int& length_)
{
    const void* ptr = GetOne(index_, length_);
    ...
    d4_assert(((const t4_wchar*) ptr)[(length_>>1)-1] == 0);
    return ptr;
}

void c4_FormatW::Set(int index_, const c4_Bytes& buf_)
{
    int m = buf_.Size();
    if (--m >= 0)
    {
...
int c4_FormatW::DoCompare(const c4_Bytes& b1_, const c4_Bytes& b2_)
{
    c4_WideString v1 ((const t4_wchar*) b1_.Contents(), b1_.Size());
    c4_WideString v2 ((const t4_wchar*) b2_.Contents(), b2_.Size());
    
    return v1.CompareNoCase(v2);
}

void c4_FormatW::Insert(int index_, const c4_Bytes& buf_, int count_)
{
    d4_assert(count_ > 0);
...
c4_Handler* f4_CreateFormat(const c4_Property& prop_, c4_HandlerSeq& seq_)
{
  switch (prop_.Type()) {
    ...
    case 'W': return d4_new c4_FormatW (prop_, seq_);
...
int f4_CompareFormat(char type_, const c4_Bytes& b1_, const c4_Bytes& b2_)
{
  switch (type_) {
    ...
    case 'W': return c4_FormatW::DoCompare(b1_, b2_);
