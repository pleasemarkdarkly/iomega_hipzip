// viewx.cpp --
// $Id: viewx.cpp,v 1.10 2002/03/13 22:10:00 wcvs Exp $
...
c4_WideStringRef::operator const t4_wchar* () const
{
    //d4_assertThis;
	...
	if (result.Size() > 0)
		return (reinterpret_cast <const t4_wchar*> (result.Contents()));
	else
		return (t4_wchar*)0;
}

inline unsigned int wstrlen(const t4_wchar* wstr)
{
	if (wstr)
	{
		int ret = -1;
...
c4_WideStringRef& c4_WideStringRef::operator= (const t4_wchar* value_)
{
    //d4_assertThis;
    
    SetData(c4_Bytes (value_, 2*wstrlen(value_) + 2));
    return *this;
}
